...
class ClientCode(Ui_MainWindow, QMainWindow):
    ...
    def send_message(self, message_type, message_content):
        ...
        message = f"{message_type}:{self.username} > {message_content}\n".encode('utf-8')
        message_header = f'{len(message):< {HEADER_LENGTH}}'.encode('utf-8')
        self.sock.send(message_header + message)
    ...
    def write(self):
        ...
        message_content = self.textEdit.toPlainText()
        self.send_message(message_type, message_content)
        plaintext = "Your message to be encrypted"
        ciphertext, iv = encrypt_message(plaintext.encode(), encryption_key)

      
        # current_time = time.time()
        # time_since_last_message = current_time - self.last_message_sent_time
        # if time_since_last_message < 1 / MESSAGE_SEND_RATE_LIMIT:
        #     print("Message send rate exceeded. Queuing message...")
        #     # Queue the message for later sending
        #     self.queue_message()
        # else:
        #     # Send the message immediately
        #     self.send_message()
        #     self.last_message_sent_time = current_time
    ...
    def send_queued_messages(self):
        """Send messages from the queue at a controlled rate"""
        current_time = time.time()
        time_since_last_message = current_time - self.last_message_sent_time
        if time_since_last_message >= 1 / MESSAGE_SEND_RATE_LIMIT:
    ...
    def send_image(self, open_file=None):
        ...
        if open_file[0]:
            with open(open_file[0], 'rb') as file:
                ...
                self.send_message(message_type, image_data)
    ...
    def receive(self):
        try:
            while True:
                ...
                message_type_length = int(message_header.decode('utf-8').strip())
                message_type = self.sock.recv(message_type_length).decode('utf-8')
                # Get USERNAME
                username_header = self.sock.recv(HEADER_LENGTH)
                ...
                if message_type == MESSAGE_TYPE_TEXT:
                    self.handle_text_message(username, message_content)
                elif message_type == MESSAGE_TYPE_IMAGE:
                    self.handle_image_message(username, message_content)
                elif message_type == MESSAGE_TYPE_SYSTEM:
                    self.handle_system_message(username, message_content)
                else:
    ...
    def handle_text_message(self, username, message_content):
        # Process text message
        print("Text message from", username, ":", message_content)

    def handle_image_message(self, username, image_data):
        # Process image message
        print("Image message from", username, ":", len(image_data), "bytes")

    def handle_system_message(self, username, message_content):
        # Process system message
        print("System message from", username, ":", message_content)
